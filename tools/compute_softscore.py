...
def filter_answers(answers_dset, min_occurence):
    ...
    occurence_cp = copy.copy(occurence)
    for answer in occurence_cp.keys():
        if len(occurence_cp[answer]) < min_occurence:
...
def load_cp(args):
    train_answer_file = args.annotation_path + "/vqacp_v2_train_annotations.json"
    with open(train_answer_file) as f:
        train_answers = json.load(f)  # ['annotations']

    val_answer_file = args.annotation_path + "/vqacp_v2_test_annotations.json"
    with open(val_answer_file) as f:
        val_answers = json.load(f)  # ['annotations']

    occurence = filter_answers(train_answers, 9)
    path = args.data_path + 'cpv2'
    ans2label = create_ans2label(occurence, 'trainval', path)
    compute_target(train_answers, ans2label, 'train', path)
    compute_target(val_answers, ans2label, 'val', path)


def load_cp_v1(args):
    train_answer_file = args.annotation_path + "/vqacp_v1_train_annotations.json"
    with open(train_answer_file) as f:
        train_answers = json.load(f)  # ['annotations']

    val_answer_file = args.annotation_path + "/vqacp_v1_test_annotations.json"
    with open(val_answer_file) as f:
        val_answers = json.load(f)  # ['annotations']
...
def load_v2(args):
    train_answer_file = args.annotation_path + \
        '/v2_mscoco_train2014_annotations.json'
    with open(train_answer_file) as f:
        train_answers = json.load(f)['annotations']

    val_answer_file = args.annotation_path + '/v2_mscoco_val2014_annotations.json'
    with open(val_answer_file) as f:
        val_answers = json.load(f)['annotations']
...
def main():
    ...
    parser.add_argument("--annotation_path", default='/data_dir/annotations')
    parser.add_argument("--data_path", default='/data_dir/data/')
